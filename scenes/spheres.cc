...
Spheres::Spheres() {
  ...
  SDF* s3 = new Sphere(vec3( 4.5, 2.0, 20), 3.5, Material(colors::GREEN, 0., 0.45, 0.5, 0.0, 1.0, 200));

  modifiable_rendering_params().reflection_depth = 4;
  modifiable_rendering_params().roughness_iterations = 5;
  modifiable_rendering_params().camera_settings.eye_pos = vec3(-15, 15, -10);
